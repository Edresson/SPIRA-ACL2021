# code adapted from https://github.com/jvbalen/hpcp_demo/blob/master/HPCP.py
...
def pitch_profile(X, F, peaks, bins_per_octave):
    ...
    pitch = hz2midi(F)
    pitch_in_bins = bins_per_octave * pitch / 12 # !!! troquei a divisao normal pela divisao com floor pra arrumar um bug sem saber o que estou fazendo 100%, conferir c quem manja
    ...
    #print(X.dtype)
    T = T.astype(np.int64)
    pitch_in_bins = pitch_in_bins.astype(np.int64)
    #print(T.dtype)
    #rint(pitch_in_bins.dtype)
...
class HPCP(nn.Module):                                                                        # in dB below the highest peak  # in dB below 0
    def __init__(self, sample_rate, win_size=4096, hop_size=1024, window='blackman', precision='float32', f_min=100, f_max=5000, global_thr=80,  local_thr=30, bins_per_octave=120, whitening=True, filter_width=1/3., harmonic_decay=0.6, harmonic_tolerance=2/3.,  norm_frames=False, final_thr=0.): 
        
        super(HPCP, self).__init__()
    ...
    def forward(self, waveform):
        ...
        y = waveform.numpy()
        #print(y.shape)
        Y, k, f, t = stft(y, self.sample_rate, win_size=self.win_size, hop_size=self.hop_size, window=self.window, precision=self.precision)

        # prune spectrogram to [f_min, f_max]
        Y_lim, k, f = prune_spectrogram(Y, k, f, f_min=self.f_min, f_max=self.f_max)

        # threshold spectrogram based on dB magnitudes
        Y_dB = dB(Y_lim)
        Y_thr = global_thresholding(Y_dB, thr=self.global_thr)
        if self.local_thr < self.global_thr:
            Y_thr = local_thresholding(Y_thr, thr=self.local_thr)

        # peak interpolation
        Y_pks, F, peaks = spectral_peaks(Y_thr, k, self.sample_rate, self.win_size)

        # multi-octave pitch profile based on linear magnitudes
        Y_lin = lin_mag(Y_pks, self.global_thr)
        pp = pitch_profile(Y_lin, F, peaks, self.bins_per_octave)
        if self.whitening:
            pp = whiten(pp, bins_per_octave=self.bins_per_octave, filter_width=self.filter_width)

        # harmonic summation
        hpp = sum_harmonics(pp, harmonic_decay=self.harmonic_decay,
                            harmonic_tolerance=self.harmonic_tolerance,
                            bins_per_octave=self.bins_per_octave)

        # fold to chromagram/hpcp
        pcp = fold_octaves(hpp, bins_per_octave=self.bins_per_octave)
        if self.norm_frames:
            pcp = normalize_frames(pcp, self.final_thr)
